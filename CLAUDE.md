...
S&P 500 (~503 支)
  ...
  ↓ Step 6   ranker.py       L3 DeepSeek AI 精選（≤3 支；30 欄 Markdown 表含 RS_vs_Sector、基本面欄位、空頭比例、ATR14；每產業 ≤8 支）
             ...
             tracker.py      訊號追蹤（watchlist.json）→ 結算歸檔（performance_history.json）
                              組合層持倉上限 MAX_ACTIVE_POSITIONS=5：事前掛單名單制，僅優先序
                              （AI 信心 → L2 分數）前 free_slots 名可進場（DD-20 v2）
                              動能/突破策略到期時收盤仍貼近峰值（回撤 <3%）獲有界延長
                              （最多再 10 個交易日），延長期間止損/停利/移動停利照常優先（DD-21）
             publisher.py    HTML 報告 → GitHub Pages（個股浮損益、今日結算區段、策略 Tooltip、歷史績效儀表板、
                              明日掛單計畫區段＝名單制的前端視圖，DD-9）
```
...
34. **組合層級 active 持倉上限：事前掛單名單制（tracker.py DD-20、publisher.py DD-8/DD-9）**：watchlist 持倉數原本無組合層上限（每日 L3 流入 ≤3 支 × DD-19 淺回檔帶 ~100% 觸價成交率 × 常見 15 日持有期 ≈ 穩態 45 支同時持倉），與使用者真實資金操作脫節，績效統計隱含「資金無限」假設。新增 `MAX_ACTIVE_POSITIONS`（env，預設 5）與純函式 `compute_order_plan()`：`free_slots = max(0, 上限 − active 數)`，`roster` = 全部 watch 條目依 `_slot_priority_key`（`-ai_confidence` → `-l2_score` → `symbol`，缺值視為 0）排序，`eligible` = 前 `free_slots` 名。**進場資格為事前名單制（v2）**：`run_tracker()` E 步驟開頭算一次 `eligible`，watch→active 僅限名單內條目——對應使用者「依前晚報告隔日只掛前 N 名限價單」的真實操作；初版「事後擇優」（觸價者中信心高者得名額）會把使用者沒掛單的交易寫進績效（帳實不符，與 DD-17/DD-19 同類缺陷），已修訂。名單外觸價者以 `today_low=None` 重跑 `_eval_status()` 取收盤價判定——失效（跌破止損/已追高）直接作廢（沒掛單即無真實交易），否則維持 watch 並設 `slot_blocked_today` 旗標（迴圈頂端每日重置且早於 `sym not in latest` 的 continue；B/C `base` 字典含 `False` 預設）。E 結尾對最終 watchlist 再算一次寫入 `categories["order_plan"]`，純函式確定性保證「報告名單 = 次日進場資格」，不需持久化。當日結算不退還名額（1-day lag）；超額持倉不強平；B/C 新訊號照常入 watch；DD-19 dormant 分支因 blocked 重跑而重新可達，不得清理。publisher 端：**明日掛單計畫區段（DD-9）**由 `_order_plan_section()` 渲染 `order_plan`——名額註記（0 時顯示「持倉已滿，明日不建議掛新單」）、依優先序列出排名/掛單價（`buy_zone_upper`）/止損/目標/信心/L2/剩餘觀察天數，前 `free_slots` 名標「✅ 建議掛單」餘標「⏸ 備援」；留意清單列補「信心 X/10｜L2 Y 分」；被擋 watch 條目標註「今日觸價但未在掛單名單，未進場」（DD-8）；今日統計顯示「持倉/上限」、有效追蹤清單標題附「上限 N 支」，常數自 `tracker` import（單一事實來源）；`_INFO_HTML` 僅寫靜態文字不插值 runtime 常數（避免 `.env` 不同的環境跑 index.html 全等守門測試誤紅）。注意：`specs/ranker.md` 另有編號相同但無關的 DD-20（L3 精選上限 5→3，見第 33 條）。→ 詳見 `specs/tracker.md`、`specs/publisher.md`、`plans/2026-07-10-max-active-positions-cap.md`（v1）、`plans/2026-07-10-order-plan-roster.md`（v2）
...
35. **到期趨勢延伸出場（tracker.py DD-21、publisher.py DD-11）**：15 筆結算績效顯示盈虧比倒掛（實現約 1:0.82，計畫 1:2.5）：8 筆獲利中 7 筆是 `FORCE_EXPIRED` 到期砍在小賺（平均 +2.96%，PCAR 出場時仍在上升趨勢 +7.61%），僅 1 筆真正觸及目標價；虧損單平均 -4.63% 幾乎全額吃滿止損。修法：`_check_settlement()` 第 5 段（持倉到期）改為五分支——僅動能/突破策略（白名單制）、未達 `hold_period + EXPIRY_EXTENSION_MAX_DAYS`（10 個交易日）硬上限、`highest_close_since_active` 存在、且收盤自峰值回撤 `< EXPIRY_TRAIL_RETRACE_PCT`（3%）者，到期時獲延長（回傳 `None`，明日再判）；其餘維持原行為 `FORCE_EXPIRED` 收盤出場。到期當天即檢查回撤，已回撤達標者當天照舊出場，不多賴一天；延長期間既有優先序 1~4（黑天鵝/止損/停利/移動停利）照常先行判定。零新增 exit_reason（沿用既有 `FORCE_EXPIRED`）、零新增 watchlist 欄位（延長態完全由 `active_days > hold_period` 推導，延長天數 = `active_days - hold_period`）、函式簽名不變；三方抗辯審查後捨棄 v1 draft 的雙均線 gate（`EMA10`/`EMA20` 缺失時比較會拋 `TypeError`）、新增 exit_reason、新增三個可推導欄位、env 化門檻常數。publisher 端：`_tracking_row()` active 分支 `active_days > hold_limit` 時狀態文字改「已到期延長 第 X/10 天」並顯示延長停利線；`_settled_row()` 的 `FORCE_EXPIRED` 文案區分純到期與延長後出場；`_INFO_HTML`「⏰ 到期出場」列改寫為完整靜態說明。已知取捨（暫不處理）：固定 3% 對高 ATR 股偏緊、延長部位持續佔用 `MAX_ACTIVE_POSITIONS` 名額（忠實記帳，非 bug）。→ 詳見 `specs/tracker.md`、`specs/publisher.md`、`plans/2026-08-06-expiry-trend-extension.md`
