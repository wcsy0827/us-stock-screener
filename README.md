...
| ⏰ 到期出場 | 持倉達 AI 設定天數時，動能/突破策略若收盤仍在期間最高收盤 3% 以內，自動延長持倉（最多再 10 個交易日），期間收盤自峰值回撤 3% 即出場；反轉策略與趨勢已弱者照舊到期出場（`specs/tracker.md` DD-21） |
...
| 狀態 | 說明 |
|------|------|
| ✅ active | 當日最低價已觸及買入區間上緣（模擬限價單成交，見下方 DD-19）且在前一日報告「明日掛單計畫」的建議掛單名單內（見下方持倉上限），報告顯示「持倉 N / 持有天數 天」及彩色浮盈浮虧（相對進場價）；動能/突破策略到期時若收盤仍貼近峰值會自動延長，顯示「已到期延長 第 X/10 天」（見下方到期延伸說明） |
| 🟡 watch | 今日未觸及買入區間等待回落；或已觸價但未在掛單名單被擋下（報告標註「今日觸價但未在掛單名單，未進場」，觀察期照常倒數，次日重新競爭） |
...
**組合層持倉上限：事前掛單名單制**（`tracker.py` DD-20）：同時持倉數以 `MAX_ACTIVE_POSITIONS`（預設 5，`.env` 可調）為上限。每晚報告的「📋 明日掛單計畫」區段列出全部 watch 條目的優先序排名（AI 信心分數 → 同分比 L2 分數），前 `free_slots`（上限 − 現有持倉）名標「✅ 建議掛單」、其餘標「⏸ 備援（名額外）」——**這份名單就是系統的進場資格**：只有名單內的條目觸價才轉 active，名單外觸價一律擋下並標註「今日觸價但未在掛單名單，未進場」（觀察期照常倒數，次日重新競爭）。使用者照表掛單，`performance_history.json` 就等於真實帳本（不會出現「系統判進場、你卻沒掛單」的帳實不符）。被擋當日若收盤已跌破止損或已追高，該訊號直接作廢（沒掛單即無真實交易，不記入績效）。當日結算出場不即時釋放名額（次一交易日才釋放，與隔日掛單的實務一致）；既有持倉超過上限時不強制平倉，由結算自然收斂。

**明日掛單計畫區段**（`publisher.py` DD-9）：每列提供人工下單所需的全部資訊——排名、✅/⏸ 標記、AI 信心、L2 分數、**掛單價（買入區間上緣，即限價單價位）**、買入區間、止損、目標、剩餘觀察天數；標題顯示「明日可進場名額 N 支」，名額 0 時顯示「持倉已滿，明日不建議掛新單」。留意清單各列亦附「信心 X/10｜L2 Y 分」供對照。

**到期趨勢延伸出場**（`tracker.py` DD-21）：歷史結算數據顯示到期出場常把仍在趨勢中的獲利部位砍在小賺，虧損單卻已吃滿止損，盈虧比倒掛。修法：動能/突破策略持倉到期時，若收盤仍在期間最高收盤 3% 以內，自動延長持倉（最多再 10 個交易日），延長期間收盤自峰值回撤達 3% 才出場；反轉策略與趨勢已轉弱者（回撤 ≥3%）維持原「到期即出場」行為，不延長。延長期間止損/停利/移動停利照常優先於到期判斷。報告上延長中的持倉顯示「已到期延長 第 X/10 天」與「延長停利線」；延長部位持續佔用 `MAX_ACTIVE_POSITIONS` 名額（真實持倉未變，忠實記帳，非 bug）。
...
- `FORCE_EXPIRED`：持倉天數 ≥ AI 設定的持有天數（出場價 = 收盤價）；動能/突破策略若收盤仍貼近峰值（回撤 <3%）會先獲有界延長（最多再 10 個交易日），延長期間收盤自峰值回撤 ≥3% 才觸發（見 `specs/tracker.md` DD-21）
...
MAX_ACTIVE_POSITIONS=5    # 同時持倉上限（事前掛單名單制：僅優先序前 N 名可進場，見報告「明日掛單計畫」）
```
